/**
 * 공지사항 편집 페이지 — 강좌 내 공지사항 제목과 블록 본문을 수정하는 클라이언트 컴포넌트.
 */
"use client";
...
import { getAnnouncement, saveAnnouncement } from "@/lib/api";
...
export default function AnnouncementEditPage() {
    ...
    const save = useCallback(async () => {
        ...
        setSaving(true);
        try {
            const saved = await saveAnnouncement(item);
            setItem(saved);
            setDirty(false);
            toast.success("공지사항을 저장했어요.");
        } catch (e) {
            ...
            <header className="sticky top-0 z-40 border-b bg-background/90 backdrop-blur">
                <div className="mx-auto flex h-16 w-full max-w-6xl items-center gap-3 px-4 sm:px-6">
                    ...
                    <Input
                        value={item.title}
                        onChange={(e) => {
                            setItem((p) => (p ? { ...p, title: e.target.value } : p));
                            setDirty(true);
                        }}
                    ...
                    <div className="ml-auto flex items-center gap-2">
                        <Button
                            ...
                            render={
                                <Link
                                    href={`/announcements/${item.id}?program=${item.programId}`}
